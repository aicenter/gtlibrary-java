 ...
 */
public class MDPFBRActionOrdering extends MDPFristBetterResponse {
...
//    public static boolean PRUNING = false;
    ...
    protected Map<MDPState, TreeSet<MDPAction>> orderedActions = new HashMap<MDPState, TreeSet<MDPAction>>();

    public MDPFBRActionOrdering(MDPConfig config, Player player) {
        super(config, player);
    }
...
//        MDPLowerBound = getLowerBound(myStrategy.getRootState(), myStrategy, opponentStrategy);
    ...
    private Pair<Pair<Double, Double>, Boolean> calculateBRValue(MDPState state, MDPStrategy myStrategy, MDPStrategy opponentStrategy, double alpha, double probability) {
        ...
        final TreeSet<MDPAction> orderedActions = new TreeSet<MDPAction>(new Comparator<MDPAction>() {
            @Override
            public int compare(MDPAction action, MDPAction action2) {
                int multiplier = getPlayer().getId() == 0 ? 1 : -1;
                if (actionValues.get(action) < actionValues.get(action2)) return 2*multiplier;
                else if (actionValues.get(action) > actionValues.get(action2)) return -2*multiplier;
                else if (action.equals(action2)) return 0;
                else return 1;
            }
        });
        ...
        Iterable<MDPAction> actions = (this.orderedActions.containsKey(state)) ? this.orderedActions.get(state) : myStrategy.getAllActions(state);
        ...
        if (bestAction != null) {
            if (!SAVE_DEF && !changed && !myStrategy.hasStateASuccessor(state) && bestAction.equals(myStrategy.getAllActions(state).get(0))) changed = false;
            else changed = true;
            if (changed) {
            ...
            if (orderedActions.size() == myStrategy.getAllActions(state).size())
                this.orderedActions.put(state, orderedActions);
        } else {
