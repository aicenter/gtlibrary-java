...
public class GenSumISMCTSAlgorithm implements GamePlayingAlgorithm {
    ...
    public GenSumISMCTSAlgorithm(Player searchingPlayer, Simulator simulator, BackPropFactory fact, GameState rootState, Expander expander) {
    ...
    protected double[] iteration(Node node) {
        if (node instanceof LeafNode) {
            return ((LeafNode) node).getUtilities();
        } else {
            ...
            if (child != null) {
                ...
                retValue = simulator.simulate(child.getGameState());
            }
            if (selector != null)
                selector.update(selActionIdx, retValue[n.getGameState().getPlayerToMove().getId()]);
            return retValue;
        }
    }
    ...
    private int fillBelief(InnerNode n, MCTSInformationSet curIS, double p, int nextPos) {
        ...
        if (n.getGameState().getPlayerToMove() == curIS.getPlayer()) {//searching player's node
            if (n.getInformationSet().getPlayersHistory().size() != curIS.getPlayersHistory().size() - 1)
                return nextPos; //this is after opponent's move out of current IS
            Node child = n.getChildOrNull(curIS.getPlayersHistory().getLast());
            if (child == null)
                return nextPos;//running out of the tree in memory, alternativly, we could add the nodes to the tree
            if (child instanceof LeafNode)
                return nextPos;//if based on unknown information the last players action leads to a leaf
