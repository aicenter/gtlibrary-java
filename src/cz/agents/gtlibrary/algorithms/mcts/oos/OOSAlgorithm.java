...
//        SMJournalExperiments.buildCompleteTree(oos.getRootNode());
    ...
    private static void runIR() {
        GameState root = new CCGameState();
        MCTSConfig config = new MCTSConfig();
        Expander<MCTSInformationSet> expander = new CCExpander<>(config);
        expander.getAlgorithmConfig().createInformationSetFor(root);


        OOSAlgorithm oos = new OOSAlgorithm(CCGameInfo.FIRST_PLAYER, new OOSSimulator(expander, seed), root, expander);
        SMJournalExperiments.buildCompleteTree(oos.getRootNode());
        for (int i = 0; i < 1000; i++) {
            oos.runIterations(2);
            Strategy strategy0 = StrategyCollector.getStrategyFor(oos.getRootNode(), root.getAllPlayers()[0], new MeanStratDist());
    ...
    private static void runML() {
        GameState root = new MLGameState();
        MCTSConfig config = new MCTSConfig();
        Expander<MCTSInformationSet> expander = new MLExpander<>(config);
        expander.getAlgorithmConfig().createInformationSetFor(root);


        OOSAlgorithm oos = new OOSAlgorithm(MLGameInfo.FIRST_PLAYER, new OOSSimulator(expander, seed), root, expander);
        SMJournalExperiments.buildCompleteTree(oos.getRootNode());
        for (int i = 0; i < 1000; i++) {
            oos.runIterations(200000);
            Strategy strategy0 = StrategyCollector.getStrategyFor(oos.getRootNode(), root.getAllPlayers()[0], new MeanStratDist());
        ...
        gambit.write("MLOOSDomain.gbt", root, expander);
    }

    private static void runLR() {
        GameState root = new LRGameState();
        MCTSConfig config = new MCTSConfig();
        Expander<MCTSInformationSet> expander = new LRExpander<>(config);
        expander.getAlgorithmConfig().createInformationSetFor(root);


        OOSAlgorithm oos = new OOSAlgorithm(LRGameInfo.FIRST_PLAYER, new OOSSimulator(expander, seed), root, expander);
        SMJournalExperiments.buildCompleteTree(oos.getRootNode());
        for (int i = 0; i < 1000; i++) {
            oos.runIterations(2);
            Strategy strategy0 = StrategyCollector.getStrategyFor(oos.getRootNode(), root.getAllPlayers()[0], new MeanStratDist());
        ...
        gambit.write("LROOSDomain.gbt", root, expander);
    }
