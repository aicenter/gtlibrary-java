...
//    }
    ...
    protected Iterable<Sequence> getBrokenStrategyCauses(Map<InformationSet, Map<Sequence, Double>> strategy, LPData lpData) {
        ...
        if (set == null)
            return null;
//        Map<Sequence, Double> expectedValues = getLeaderExpectedValues(behavioralToRealizationPlan(getBehavioralStrategy(lpData, leader)), behavioralToRealizationPlan(getBehavioralStrategy(lpData, follower)), shallowestBrokenStrategyCause);
        Map<Sequence, Double> expectedValues = getFollowerExpectedValues(lpData, shallowestBrokenStrategyCause);

        return sort(expectedValues, set.getOutgoingSequences());
    }
    ...
    private Map<Sequence, Double> getFollowerExpectedValues(LPData lpData, Map<Sequence, Double> brokenStrategyCause) {
        Map<Sequence, Double> followerExpectedValues = new HashMap<>(brokenStrategyCause.size());

        for (Sequence sequence : followerExpectedValues.keySet()) {
            followerExpectedValues.put(sequence, getExpectedValue(lpData, sequence));
        }
        return followerExpectedValues;
    }
    ...
    protected Iterable<Sequence> sort(final Map<Sequence, Double> shallowestBrokenStrategyCause, final Collection<Sequence> outgoing) {
        ...
        Collections.sort(list, new Comparator<Sequence>() {
            ...
            public int compare(Sequence o1, Sequence o2) {
                ...
                if (o1Value == null)
                    o1Value = Double.NEGATIVE_INFINITY;
                if (o2Value == null)
                    o2Value = Double.NEGATIVE_INFINITY;
