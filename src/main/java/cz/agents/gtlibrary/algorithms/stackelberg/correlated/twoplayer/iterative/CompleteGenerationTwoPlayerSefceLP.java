...
//                        if (relevantForFollowerInP.containsKey(leaderSequence) && relevantForFollowerInP.get(leaderSequence).contains(set.getPlayersHistory())) {
    ...
    protected void addFollowerSequencesToRG(HashSet<Sequence> followerSequences){
...
//        lpTable.setConstraintType("initP", 1);
        ...
        for (Sequence followerSequence : followerSequences){
            followerRG.add(followerSequence);
            // 6
...
//                Sequence subSeq = followerSequence.getSubSequence(followerSequence.size()-1);
                lpTable.setConstraint(followerSequence.getSubSequence(followerSequence.size() - 1), varKey, -1);
            }
            ...
            varKey = new Pair<>(followerSequence.getLastInformationSet(), followerSequence);
